...
Options:
  ...
  --preview         Open the PDF for preview with SAMPLE watermark
...
HERE
    ...
    sub prompt($label, $default?) {
        print $default.defined ?? "$label [$default]: " !! "$label: ";
    ...
    generate-check(%check, :$preview, :$print, :$printer);
}

sub generate-check(%check, Bool :$preview = False, Bool :$print = False, Str :$printer?) {
    ...
    if $preview {
        $page.save-gfx-state;
        $page.fill-color(0.8, 0.8, 0.8);
        $page.translate($page-width / 2, $check-height / 2);
        $page.rotate(-30);
        $page.font($font-regular, 72);
        $page.text(:x(-200), :y(0), :text("SAMPLE"));
        $page.restore-gfx-state;
    }
